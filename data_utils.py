...
def load_sst2():
    ...
    #with open(f"{ROOT_DIR}/data/sst2/advglue.dev", "r") as f:
...
def load_rte():
    ...
    '''
    with open("data/rte/advglue.jsonl", "r") as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['hypothesis']
            p = myjson['premise']
...
def load_qqp():
    train_questions = []
    train_answers = []
    with open("data/qqp/train.jsonl", "r") as f:
        for line in f:
            myjson = json.loads(line)
            p = myjson['question1']
            q = myjson['question2']
    ...
    test_questions = []
    test_answers = []
    with open("data/qqp/val.jsonl", "r") as f:
        for line in f:
            myjson = json.loads(line)
            p = myjson['question1']
            q = myjson['question2']
    ...
    '''
    with open("data/qqp/advglue.jsonl", "r") as f:
        for line in f:
            myjson = json.loads(line)
            p = myjson['question1']
            q = myjson['question2']
...
def load_mnli():
    ...
    with open("data/mnli/val.jsonl", "r") as f:
        for line in f:
            ...
            test_questions.append(p + '\n' + 'question: ' + q + ' true, false, or neither?')
    '''
    with open("data/mnli/advglue.jsonl", "r") as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['hypothesis']
            p = myjson['premise']
...
def load_qnli():
    train_questions = []
    train_answers = []
    with open("data/qnli/train.jsonl", "r") as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['sentence']
            p = myjson['question']
    ...
    test_questions = []
    test_answers = []
    with open("data/qnli/val.jsonl", "r") as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['sentence']
            p = myjson['question']
            ...
            test_questions.append(p + ' ' + q)
    '''
    with open("data/qnli/advglue.jsonl", "r") as f:
        for line in f:
            myjson = json.loads(line)
            q = myjson['sentence']
            p = myjson['question']
...
def load_dataset(params):
    ...
    if params['dataset'] == 'sst2':
        ...
        params['prompt_prefix'] = ""
        params["q_prefix"] = "Question pair: "
        params["a_prefix"] = "Duplicate: "
    ...
    elif params['dataset'] == 'qnli':
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_qnli()
        params['prompt_prefix'] = ""
        params["q_prefix"] = "Question and answer: "
        params["a_prefix"] = "Entrailment: "
        params['label_dict'] = {0: ['False'], 1: ['True']}
        params['inv_label_dict'] = {'False': 0, 'True': 1}
        params['task_format'] = 'classification'
        params['num_tokens_to_predict'] = 1
